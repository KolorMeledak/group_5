def conversion(target, number, precision):
    ...
    if number == 0:
        ...
        while int_number > 0:
            print(
                f"{int_number} % {target} = {int_number // target} sisa {int_number % target}"
            )
...
while True:
    ...
    for i in conversion_target:
        print(
            f"Angka {conversion_number} dalam basis {i} adalah: {conversion(int(i), conversion_number, precision)}"
        )
